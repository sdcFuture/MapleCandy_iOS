...
class BrightnessView: UIView {
    ...
    var indicatorColor: CGColor = UIColor.blue.cgColor
    ...
    func setColorLayer(color: UIColor){
        // Init the point at the correct position
        point = getPointFromColor(color)
    ...
    func drawIndicator() {
        // Draw the indicator
        if (point != nil) {
            indicator.path = UIBezierPath(roundedRect: CGRect(x: point.x-3, y: 0, width: 6, height: self.frame.size.height+5), cornerRadius: 3).cgPath
        }
    }
