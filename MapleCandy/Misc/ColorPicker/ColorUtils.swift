...
func hsv2rgb(_ hsv: HSV) -> RGB {
    ...
    let i = Int(hsv.hue * 6)
    let f = ((hsv.hue * 6 - CGFloat(i) > 1) ? 1 : (hsv.hue * 6 - CGFloat(i)))
    let p = hsv.brightness * (1 - (hsv.saturation > 1 ? 1 : hsv.saturation))
    let q = hsv.brightness * (1 - f * (hsv.saturation > 1 ? 1:  hsv.saturation))
    let t = hsv.brightness * (1 - (1 - f) * (hsv.saturation > 1 ? 1 : hsv.saturation))
    switch (i % 6) {
