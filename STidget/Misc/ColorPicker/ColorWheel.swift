...
class ColorWheel: UIView {
    ...
    init(frame: CGRect, color: UIColor!) {
        ...
        wheelLayer = CALayer()
        wheelLayer.frame = CGRect(x: padding/2, y: padding/2, width: self.frame.width - padding, height: self.frame.height - padding)
        ...
        brightnessLayer.path = UIBezierPath(roundedRect: CGRect(x: 10, y: 10, width: self.frame.width-20, height: self.frame.height-20), cornerRadius: (self.frame.height-20)/2).cgPath
    ...
    func pointAtHueSaturation(_ hue: CGFloat, saturation: CGFloat) -> CGPoint {
        ...
        let x = dimension / 2 + radius * cos(hue * CGFloat(M_PI) * 2) + 10;
        let y = dimension / 2 + radius * sin(hue * CGFloat(M_PI) * 2) + 10;
